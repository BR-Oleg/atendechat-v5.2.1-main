      ...
      <q-item-section id="ListItemsTicket">
        ...
        <q-item-label lines="1"
          ...
          <span class="absolute-bottom-right ">
            ...
            <q-icon
              v-if="(ticket.stepAutoReplyId && ticket.autoReplyId && ticket.status === 'pending') || (ticket.chatFlowId && ticket.stepChatFlow && ticket.status === 'pending') || (ticket.is_chat_ia && ticket.status === 'pending')"
              name="mdi-robot"
              color="primary"
              size="1.8em"
              class="q-mb-sm">
