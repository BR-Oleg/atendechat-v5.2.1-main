...
import VerifyMensageOpenIA from "../../OpenIA/VerifyMensageOpenIA";
  ...
  return new Promise<void>((resolve, reject) => {
    (async () => {
      ...
      try {
        ...
        let menssage;
        if (msg.hasMedia) {
          menssage = await VerifyMediaMessage(msg, ticket, contact);
        } else {
          menssage = await VerifyMessage(msg, ticket, contact);
        }

        const isBusinessHours = await verifyBusinessHours(msg, ticket);

        // await VerifyAutoReplyActionTicket(msg, ticket);
        // if (whatsapp.is_open_ia) {
        //   if (isBusinessHours) await VerifyMensageOpenIA(menssage, msg, ticket, whatsapp);
        // } else {
        ...
        if (isBusinessHours && ticket.queueId && ticket.is_chat_ia) {
          ...
          await VerifyMensageOpenIAQueue(ticket, queue, false, true, msg, menssage);
        }
        // }
